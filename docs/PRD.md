...
```
1.  관리자 로그인 (/admin)
    - 비로그인 상태로 /admin 접속 시 사이드바 없이 로그인 UI 표시
    - 이메일/비밀번호 또는 Google OAuth로 로그인
    - 로그인 후 is_admin 검증:
        → is_admin = true: 사이드바 포함 대시보드 진입
        → is_admin = false: 접근 거부 메시지 표시 (사이드바 없음)
    ✅ 구현 완료 (AdminLoginPage, layout.tsx is_admin 체크)
...
3.  [분기 A] 이벤트 관리
    ...
    → 강제 취소 (status = 'cancelled') — 수정은 주최자 전용 기능으로 관리자 불가

    [분기 B] 사용자 관리
    → /admin/users → 전체 사용자 목록 (이메일·가입일·생성 모임수·참여 모임 수)
    → is_admin 토글 (권한 부여/회수) / 계정 비활성화
...
- 이벤트 관리: 전체 모임 목록 조회·강제 취소 (수정은 주최자 전용, 관리자 불가)
...
- 대시보드(/admin): 총 모임 수, 이번 달 신규 모임, 총 사용자 수, 이번 달 신규 가입 지표 카드 4개 + 최근 모임·최근 가입 사용자 2열 나란히 배치 (각 5개 미리보기, "모두 보기" 버튼으로 전체 목록 이동)
- 이벤트 관리(/admin/events): 전체 모임 테이블 (제목·주최자·날짜·상태·참가 인원·초대링크 코드), 제목 검색·상태 필터, 강제 취소 Dialog (수정 기능 없음 — 주최자 전용)
- 사용자 관리(/admin/users): 전체 사용자 테이블 (아바타·이름·이메일·가입일·생성 모임수·참여 모임 수), is_admin 토글 Switch, 이름/이메일 검색
...
profiles
  ...
  - full_name (text)
  - avatar_url (text)
  - is_admin (boolean, default false)    -- 플랫폼 관리자 여부 ✅ DB 컬럼 추가 완료
  - created_at (timestamptz)
```

> **관리자 권한 설정**: Supabase 대시보드 → Table Editor → profiles 테이블에서 `is_admin` 컬럼을 직접 `true`로 편집하여 관리자 권한 부여/회수
...
| 기능 ID | 기능명             | 설명                                                                                                     | 관련 페이지     | 현재 상태  |
| ------- | ------------------ | -------------------------------------------------------------------------------------------------------- | --------------- | ---------- |
| F016    | 관리자 이벤트 관리 | 전체 모임 목록 조회(검색·필터·초대링크 코드 표시), 강제 취소 (수정은 주최자 전용)                        | `/admin/events` | 🔄 UI 완료 |
| F017    | 관리자 사용자 관리 | 전체 사용자 목록(생성 모임수·참여 모임 수 포함), is_admin 토글(권한 부여/회수), 계정 비활성화            | `/admin/users`  | 🔄 UI 완료 |
| F018    | 관리자 통계 분석   | 기간별 신규 가입자·모임 생성·참가 신청·카풀·정산 이용률 통계 차트                                        | `/admin/stats`  | 🔄 UI 완료 |
| F020    | 관리자 대시보드    | 총 모임 수·사용자 수·이번 달 신규 지표 카드, 최근 모임·최근 가입 사용자 2열 미니 테이블 (모두 보기 연결) | `/admin`        | 🔄 UI 완료 |
...
└── /admin                              # 관리자 전용 (is_admin = true, 비관리자 접근 시 리다이렉트)
    ...
    ├── /admin/events                   # 이벤트 관리 (전체 모임 목록·초대링크 코드·강제 취소, 수정 불가)
