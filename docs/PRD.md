...
```
1.  홈 페이지 (/)
    ↓ Google 로그인 (이미 로그인 시 자동 건너뜀)

2.  주최자 대시보드 (/protected)
    ↓ "모임 만들기" 버튼 클릭

3.  모임 생성 페이지 (/protected/meetings/new)
    ↓ 제목·날짜·장소·최대 인원·참가비·승인 방식 입력 후 "저장" 버튼
...
6.  참가 신청 관리 (?tab=participants)
    ↓ 신청자 목록에서 승인/거절 처리 → 대기자 순번 자동 관리

7.  모임 운영 (탭 전환)
    ↓ 공지사항 작성·수정 (?tab=notices) / 카풀 현황 확인 (?tab=carpool)

8.  정산 입력 (?tab=settlement)
    ↓ 비용 항목 입력 → 1/N 또는 수동 분배 → 납부 현황 체크

9.  [완료] → 모임 수정/취소 또는 대시보드로 돌아가기
```
...
1.  초대 링크 접속 (/invite/[token])
    ↓ 모임 기본 정보 표시 (제목·날짜·장소·참가비)

2.  [분기 A] 비로그인 상태
    → "Google로 로그인하여 신청" 버튼 클릭
    → Google OAuth 로그인 / 최초 접속 시 자동 계정 생성

    [분기 B] 이미 로그인 상태
    → participations에서 본인 신청 여부 자동 확인
    → 신청됨: 현재 상태 표시 (승인 / 대기[순번] / 거절) → 여정 종료
    ↓
...
5.  모임 상세 페이지 (/protected/meetings/[id])
    ↓ (카풀 활성 시) 드라이버 등록 또는 동승 신청 (?tab=carpool)

6.  정산 확인 (?tab=settlement)
    ↓ 본인 납부 금액 조회
...
- 하단 고정 네비게이션 바 (로그인 전/후 모든 페이지에 표시)
    - 홈 → `/protected` (대시보드)
    - 모임 → `/protected/meetings/new` (새 모임 만들기)
    - 알림 → 향후 구현 예정
...
- 사이드바 네비게이션 레이아웃 (이벤트 관리 / 사용자 관리 / 통계 분석)
...
| 기능 ID | 기능명            | 설명                                                                                | 관련 페이지                                                    |
| ------- | ----------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| F001    | 구글 로그인       | 구글 OAuth를 통한 소셜 로그인 및 로그아웃 (Host/Member/Admin 공통)                  | `/auth/login`                                                  |
...
| F005    | 초대 링크 접속    | 비로그인 시 모임 기본 정보 미리보기, Google 로그인 후 참가 신청 가능                | `/invite/[token]`                                              |
...
| 기능 ID | 기능명             | 설명                                                              | 관련 페이지     |
| ------- | ------------------ | ----------------------------------------------------------------- | --------------- |
| F016    | 관리자 이벤트 관리 | 전체 모임 목록 조회(검색·필터), 모임 수정·강제 취소·삭제          | `/admin/events` |
| F017    | 관리자 사용자 관리 | 전체 사용자 목록, is_admin 토글(권한 부여/회수), 계정 비활성화    | `/admin/users`  |
| F018    | 관리자 통계 분석   | 기간별 신규 가입자·모임 생성·참가 신청·카풀·정산 이용률 통계 차트 | `/admin/stats`  |
