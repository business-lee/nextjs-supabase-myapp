...
- ✅ **Task 017: 정산 현황 공유 및 부가 기능 구현** `[공통]`
    - ✅ 정산 현황 공유 링크 생성 기능 (F015)
        - ✅ 공개 정산 조회 페이지 라우트 추가 (`/share/settlement/[id]`)
        - ✅ `get_public_settlement` SECURITY DEFINER 함수로 anon 접근 허용
        - ✅ 정산 탭 납부 현황 카드에 공유 버튼 + 클립보드 복사 구현
    - ✅ 초대 링크 클립보드 복사 기능 개선 (복사 완료 토스트 알림 — 기존 구현 확인)
    - ✅ `/share` 경로 proxy.ts 예외 처리 추가 (비로그인 접근 허용)
    - 모임 D-day 표시 유틸리티 함수 구현 — Phase 5로 이관
    - 에러 상태 Toast 알림 전체 적용 — Phase 5로 이관
    - Skeleton 로딩 상태 전체 페이지 적용 — Phase 5로 이관
    - 빈 상태(Empty State) UI 처리 — Phase 5로 이관
...
- ✅ **Task 020: Admin 접근 제어 BE 구현** `[BE]` - 우선순위
    ...
    - ✅ Admin 대시보드 BE 연동 (`getAdminDashboardStatsAction`, 최근 모임/사용자 조회)
    - **테스트 체크리스트 (Playwright MCP)**:
...
- ✅ **Task 021: Admin 이벤트 관리 BE 연동** `[BE]` `[병렬가능]`
    - ✅ 전체 모임 조회 Server Action 구현 (`getAdminMeetingsAction`, Admin RLS 활용)
    - ✅ 모임 강제 취소 Server Action (`forceCancelMeetingAction`, `status = 'cancelled'`)
    - ✅ `app/admin/events/page.tsx` Server Component 전환, 더미 데이터 → 실제 Server Action 교체
    - ✅ URL 쿼리 파라미터 기반 검색·필터 서버 사이드 전환 (`EventsTableClient` 분리)
    - ✅ 취소된 모임에는 취소 버튼 미표시
    - **테스트 체크리스트 (Playwright MCP)**:
...
- ✅ **Task 022: Admin 사용자 관리 BE 연동** `[BE]` `[병렬가능]`
    - ✅ 전체 사용자 조회 Server Action (`getAdminUsersAction`, 생성/참여 모임 수 집계 포함)
    - ✅ `is_admin` 토글 Server Action (`toggleAdminRoleAction`, Optimistic UI 적용)
    - ✅ `app/admin/users/page.tsx` Server Component 전환, 더미 데이터 → 실제 Server Action 교체
    - ✅ URL 쿼리 파라미터 기반 검색 서버 사이드 전환 (`UsersTableClient` 분리)
    - ✅ 계정 비활성화: "준비 중" 안내 다이얼로그로 대체 (DB 컬럼 미존재)
    - **테스트 체크리스트 (Playwright MCP)**:
        - ✅ Admin 계정으로 전체 사용자 목록 조회 확인
        - [ ] is_admin 토글 후 대상 계정 권한 변경 확인 (새로고침 유지)
        - [ ] 비관리자 계정의 `/admin/users` 접근 차단 확인

- ✅ **Task 023: Admin 통계 분석 BE 연동** `[BE]` `[병렬가능]`
    - ✅ 월별 신규 가입자 집계 (`getAdminChartDataAction` — 최근 6개월 집계)
    - ✅ 월별 모임 생성·참가 신청 추이 집계
    - ✅ 기능 이용률 집계 (카풀 활성화/정산 생성 기반)
    - ✅ `app/admin/stats/page.tsx` Server Component 전환, 더미 데이터 → 실제 데이터 교체
    - ✅ 기간 필터는 클라이언트 슬라이싱 방식 유지 (`StatsChartClient` 분리)
    - **테스트 체크리스트 (Playwright MCP)**:
        - ✅ 통계 차트 실제 데이터 기반 렌더링 확인
...
- ✅ **Task 019: Vercel 배포 및 프로덕션 환경 검증** `[공통]` - 우선순위
    ...
    - ✅ Supabase Auth Redirect URLs에 `https://nextjs-supabase-myapp.vercel.app/**` 등록
    - ✅ Google Cloud Console OAuth 승인된 리디렉션 URI 등록
    - ✅ `main` 브랜치 push → 프로덕션 자동 배포 트리거 확인
    - ✅ React hydration 오류(#418) 수정 — 날짜 포매팅 함수에 `timeZone: "Asia/Seoul"` 명시 적용
        - ✅ `components/meeting-card.tsx` — `formatEventDate()` 타임존 고정
        - ✅ `app/(main)/protected/meetings/[id]/meeting-detail-client.tsx` — `formatEventDate()` 타임존 고정
        - ✅ `components/invite-view.tsx` — `formatEventDate()` 타임존 고정
    - **테스트 체크리스트 (Playwright MCP)**:
        - ✅ `.vercel.app` 도메인 접속 후 핵심 화면 렌더링 정상 확인
        - ✅ 구글 로그인 OAuth 리다이렉트 URL 프로덕션 도메인 설정 확인
        - ✅ Supabase Storage CDN 썸네일 이미지 정상 로드 확인
        - ✅ 이메일 로그인 → `/protected/meetings` 정상 이동 확인
        - ✅ 초대 링크(`/invite/[token]`) 주최자명·참가 상태 표시 확인
        - ✅ `/admin` 관리자 대시보드 및 이벤트 관리 정상 확인
...
| F015    | 정산 현황 공유                                          | ✅ Task 017           | ✅ 완료      |
| F016    | 관리자 이벤트 관리 (강제 취소·초대링크 조회, 수정 제외) | ✅ Task 008, Task 021 | ✅ 완료      |
| F017    | 관리자 사용자 관리 (생성·참여 모임수 포함)              | ✅ Task 009, Task 022 | ✅ 완료      |
| F018    | 관리자 통계 분석                                        | ✅ Task 010, Task 023 | ✅ 완료      |
| F019    | 프로필 관리                                             | ✅ Task 003, Task 012 | 🔄 일부 완료 |
| F020    | 관리자 대시보드 UI                                      | ✅ Task 007, Task 020 | ✅ 완료      |
