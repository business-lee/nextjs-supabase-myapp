...
- 모바일 컨테이너 레이아웃: 최대 너비 530px 중앙 정렬, 외부 배경 연한 회색 (`app/layout.tsx`)
- 하단 고정 네비게이션 바 골격 (`components/bottom-nav.tsx`): 홈·모임·알림·프로필·설정 5개 메뉴, 로그인 전/후 전체 페이지 표시
...
- **Task 003: 공통 컴포넌트 및 레이아웃 구현** `[UI]`
    ...
    - ✅ 하단 고정 네비게이션 바 골격 구현 (홈·모임·알림·프로필·설정, 경로 기반 활성 상태)
    - 하단 네비게이션 바 고도화 (로그인 사용자 아바타, 로그아웃 연동)
