...
### Phase 2 Admin UI (Task 007~010): 구현 예정

- `/admin` 대시보드: 주요 지표 카드 UI, 최근 모임 테이블
- `/admin/events` 이벤트 관리: 전체 모임 테이블, 검색·필터, 수정/취소 Dialog UI
- `/admin/users` 사용자 관리: 전체 사용자 테이블, is_admin 토글 UI
...
- **Task 007: Admin 레이아웃 고도화 및 대시보드 UI 구현** `[UI]` - 우선순위
    - `app/admin/layout.tsx` 사이드바 활성 경로 하이라이트 (`usePathname` 기반) 추가
    - 사이드바 하단 현재 로그인 사용자 아바타·이름 표시 (더미 데이터)
    - 개발 모드 배너 추가 (development 환경에서만 표시: "⚠ 개발 모드: Admin 접근 제어 미적용")
    - `app/admin/page.tsx` 주요 지표 카드 4개 UI 구현 (더미 데이터 기반):
        - 총 모임 수 / 이번 달 신규 모임
        - 총 사용자 수 / 이번 달 신규 가입
    - 최근 모임 미니 테이블 (MOCK_MEETINGS 5개 활용)
    - shadcn/ui `Table` 컴포넌트 설치 (필요 시)
    - Admin 전용 더미 데이터 추가 (`lib/mock-data.ts` 하단 신규 섹션):
        - `MOCK_ADMIN_STATS`, `getMockAdminStats()` 함수

- **Task 008: Admin 이벤트 관리 UI 구현** `[UI]`
    - `app/admin/events/page.tsx` 전체 모임 목록 테이블 UI 구현 (MOCK_MEETINGS 활용)
        - 컬럼: 제목 / 주최자 / 날짜 / 상태 배지 / 참가 인원 / 액션
    - 클라이언트 사이드 검색 필터 (제목 기준) + 상태 필터 드롭다운
    - 모임 수정 Dialog UI 골격 (React Hook Form + Zod, 제출 핸들러는 Task 021에서 연동)
    - 강제 취소 확인 Dialog UI 골격
    - 빈 상태(Empty State) UI
    - 더미 데이터 추가: `getMockAdminMeetings()` 함수 (MOCK_MEETINGS 기반 파생)

- **Task 009: Admin 사용자 관리 UI 구현** `[UI]`
    - `types/database.ts`에 `AdminProfileRow` 인터페이스 추가:
        ```typescript
    ...
    - `app/admin/users/page.tsx` 전체 사용자 목록 테이블 UI 구현
        - 컬럼: 아바타 / 이름 / 이메일 / 가입일 / 참여 모임 수 / Admin 권한 토글 / 액션
...
- **Task 010: Admin 통계 분석 UI 구현** `[UI]`
    - shadcn/ui charts 설치 (`npx shadcn@latest add chart`)
...
- **Task 011: Supabase 데이터베이스 마이그레이션 및 RLS 정책 적용** `[BE]` - 우선순위
    ...
    - Migration SQL 파일 작성 (`supabase/migrations/`):
        ...
        - `meetings` (invite_token UNIQUE, carpool_enabled, approval_type, status 포함)
...
### Phase 5: 관리자(Admin) BE 연동 완성

> **목표**: Admin UI(Phase 2 Task 007~010)에서 구현한 더미 데이터를 실제 Supabase DB 연동으로 교체하고, is_admin 기반 접근 제어 완성
> **선행 조건**: Phase 3 Task 011 (DB 마이그레이션 및 is_admin 컬럼 추가) 완료
> **기간**: 3~4일

- **Task 020: Admin 접근 제어 BE 구현** `[BE]` - 우선순위
    - `app/admin/layout.tsx`의 is_admin 체크 주석 해제 (profiles.is_admin DB 조회 활성화)
    - Admin 전용 RLS 정책 보완 (`auth.jwt() ->> 'is_admin' = 'true'` 조건)
    - 미들웨어에서 `/admin/*` 경로의 is_admin 서버 사이드 검증 추가
    - 개발 모드 배너 조건 제거 (운영 환경에서 미표시)
    - 초기 Admin 계정 설정 방법 문서화 (Supabase 대시보드에서 수동 설정)
    - **선행 조건**: Task 011 완료 필요
    - **테스트 체크리스트 (Playwright MCP)**:
        - [ ] is_admin = true 계정으로 `/admin` 정상 접근 확인
        - [ ] is_admin = false 계정의 `/admin/*` 접근 시 `/protected` 리다이렉트 확인
        - [ ] 비로그인 상태의 `/admin/*` 접근 시 `/auth/login` 리다이렉트 확인
...
- **Task 021: Admin 이벤트 관리 BE 연동** `[BE]` `[병렬가능]`
    ...
    - 모임 수정 Server Action (Admin 권한으로 타 사용자 모임 수정)
...
| F002    | 모임 생성          | ✅ Task 004, Task 012 | 🔄 UI 완료      |
| F003    | 모임 대시보드      | ✅ Task 004, Task 012 | 🔄 UI 완료      |
| F004    | 초대 링크 공유     | ✅ Task 005, Task 012 | 🔄 UI 완료      |
| F005    | 초대 링크 접속     | ✅ Task 006, Task 013 | 🔄 UI 완료      |
| F006    | 참가 신청 및 승인  | ✅ Task 006, Task 013 | 🔄 UI 완료      |
| F007    | 공지사항 CRUD      | ✅ Task 005, Task 014 | 🔄 UI 완료      |
| F008    | 카풀 매칭          | ✅ Task 005, Task 015 | 🔄 UI 완료      |
| F009    | 정산 관리          | ✅ Task 005, Task 016 | 🔄 UI 완료      |
| F010    | 모임 수정          | ✅ Task 004, Task 012 | 🔄 UI 완료      |
| F011    | 모임 취소          | ✅ Task 004, Task 012 | 🔄 UI 완료      |
| F012    | 대기자 자동 승급   | Task 013              | ⏳ 대기         |
| F013    | 참여 통계 표시     | ✅ Task 005, Task 013 | 🔄 UI 완료      |
| F014    | 카풀 활성화 토글   | ✅ Task 005, Task 015 | 🔄 UI 완료      |
| F015    | 정산 현황 공유     | Task 017              | ⏳ 대기         |
| F016    | 관리자 이벤트 관리 | Task 008, Task 021    | ⏳ UI 구현 예정 |
| F017    | 관리자 사용자 관리 | Task 009, Task 022    | ⏳ UI 구현 예정 |
| F018    | 관리자 통계 분석   | Task 010, Task 023    | ⏳ UI 구현 예정 |
| F019    | 프로필 관리        | ✅ Task 003, Task 012 | 🔄 일부 완료    |
| F020    | 관리자 대시보드 UI | Task 007, Task 020    | ⏳ UI 구현 예정 |
