...
<button className="bg-violet-500 hover:bg-violet-600 focus:ring-2 focus:ring-violet-300 focus:outline-none active:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-50">
    저장
</button>;
...
<input className="border border-gray-300 placeholder:text-gray-400 invalid:border-pink-500 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-gray-50 disabled:text-gray-400" />;
```
...
<ul>
    {items.map((item) => (
        <li key={item.id} className="flex border-b py-4 first:pt-0 last:border-0 last:pb-0">
            {item.name}
        </li>
    ))}
</ul>;
...
<a href="#" className="group block rounded-lg p-4 hover:bg-sky-500">
    ...
    <span className="text-sm text-gray-400 group-hover:text-sky-200">더 보기 →</span>
</a>;
...
<ul>
    {items.map((item) => (
        <li key={item.id} className="group/item flex items-center">
            ...
            <a
                href="#"
                className="invisible ml-auto text-sm text-blue-500 group-hover/item:visible"
            >
                편집
            </a>
        </li>
...
<div className="flex flex-col gap-1">
    <input
        type="email"
        className="peer rounded border px-3 py-2"
...
@theme {
    ...
    --color-brand-dark: oklch(0.5 0.18 260);
...
<div style={{ color: "hsl(var(--primary))" }}>프라이머리 색상</div>;
```
...
@layer components {
    .btn-primary {
        @apply bg-primary text-primary-foreground hover:bg-primary/90 focus:ring-ring rounded-md px-4 py-2 focus:ring-2 focus:outline-none;
    }

    .form-input {
        @apply border-input bg-background ring-offset-background focus:ring-ring w-full rounded-md border px-3 py-2 text-sm focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50;
    }
}
...
<button className="theme-midnight:bg-black theme-midnight:text-white">미드나이트 테마 버튼</button>;
```
...
<Accordion type="single" collapsible className="w-full">
    <AccordionItem value="q1">
        ...
        <AccordionContent>일반적으로 주문 후 2-3 영업일 이내에 배송됩니다.</AccordionContent>
    </AccordionItem>
    <AccordionItem value="q2">
        <AccordionTrigger>반품 정책은 어떻게 되나요?</AccordionTrigger>
        <AccordionContent>수령 후 14일 이내에 반품 신청이 가능합니다.</AccordionContent>
    </AccordionItem>
</Accordion>;
...
<button aria-expanded={isOpen} className="aria-expanded:bg-accent">
    메뉴
</button>;
